...
UART UART_construct(uint32_t moduleInstance, uint32_t port, uint32_t pins) {
  ...
  // generation
  uart.config.parity = EUSCI_A_UART_ODD_PARITY;  // Odd Parity
...
void UART_SetBaud_Enable(UART* uart_p, UART_Baudrate baudChoice) {
 ...
 // uint32_t secondModRegMapping[NUM_BAUD_CHOICES] = {0, 0, 0, 25};

  // TODO: Replace 0s with the correct statement. Use line 71 as your guide
  uart_p->config.clockPrescalar = 26;
  uart_p->config.firstModReg = 0;
  uart_p->config.secondModReg = 214;
